...
public final class MyMedicineDocs {
	...
	@ApiResponses({
			@ApiResponse(responseCode = "201", description = "Medicine added successfully",
					content = @Content(mediaType = "application/json",
							schema = @Schema(
									implementation = com.idvey.afya.payload.response.MyMedicineResponse.class))),
	...
	@Operation(summary = "Get medicine details",
			description = "Get detailed information about a medicine including purchase history")
	@ApiResponses({
			@ApiResponse(responseCode = "200", description = "Medicine details retrieved successfully",
					content = @Content(mediaType = "application/json",
							schema = @Schema(
									implementation = com.idvey.afya.payload.response.MyMedicineDetailResponse.class))),
			@ApiResponse(responseCode = "403", description = "Access denied - not a member of the group"),
	...
	@Retention(RetentionPolicy.RUNTIME)
	@Documented
	@Operation(summary = "Search medicines in pharmacy box",
			description = "Search medicines within a specific pharmacy box")
	@ApiResponses({
	...
	@Retention(RetentionPolicy.RUNTIME)
	@Documented
	@Operation(summary = "Get pharmacy box overview",
			description = "Get complete overview of pharmacy box with all medicines")
	@ApiResponses({
			@ApiResponse(responseCode = "200", description = "Overview retrieved successfully",
					content = @Content(mediaType = "application/json", schema = @Schema(
							implementation = com.idvey.afya.payload.response.PharmacyBoxMedicinesResponse.class))),
			@ApiResponse(responseCode = "403", description = "Access denied - not a member of the group"),
			@ApiResponse(responseCode = "404", description = "Pharmacy box not found"),
			@ApiResponse(responseCode = "401", description = "Unauthorized") })
