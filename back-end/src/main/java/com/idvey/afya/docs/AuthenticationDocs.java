...
public final class AuthenticationDocs {
    ...
    @Operation(
            ...
            description = "Validates credentials and returns JWT + refresh token."
    )
    @ApiResponses({
            @ApiResponse(
                    responseCode = "200",
    ...
    @ApiResponses({
            @ApiResponse(
                    ...
                    content = @Content(
                            mediaType = "application/json",
                            schema = @Schema(implementation = com.idvey.afya.payload.response.TokenRefreshResponse.class)
                    )
            ),
