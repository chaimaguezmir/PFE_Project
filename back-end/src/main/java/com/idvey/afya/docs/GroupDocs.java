...
public final class GroupDocs {
	...
	@ApiResponses({
			@ApiResponse(responseCode = "200", description = "Group members retrieved successfully",
					content = @Content(mediaType = "application/json",
							schema = @Schema(
									implementation = com.idvey.afya.payload.response.GroupMemberResponse.class))),
	...
	@Retention(RetentionPolicy.RUNTIME)
	@Documented
	@Operation(summary = "Toggle member role",
			description = "Toggles member role between MEMBER and RESPONSIBLE (Manager only)")
	@ApiResponses({
