...
@Tag(name = "Reminders", description = "Medicine reminder management - Create, track, and update medication reminders with smart date-time scheduling")
public final class ReminderDocs {

    private ReminderDocs() {} // no instantiation
    ...
    @Operation(summary = "Get reminder start suggestions",
            description = "Provides smart suggestions for when to start reminders based on current time and selected time slots. " +
                    "**Example scenarios:** " +
                    "• Current time: 10:00 AM, reminder times: 8:00 AM, 6:00 PM " +
    ...
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "Start suggestions generated successfully",
                    content = @Content(mediaType = "application/json",
                            schema = @Schema(implementation = com.idvey.afya.payload.response.ReminderStartSuggestion.class))),
    ...
    @ApiResponses({
            @ApiResponse(responseCode = "201", description = "Reminders created successfully",
                    content = @Content(mediaType = "application/json",
                            schema = @Schema(implementation = com.idvey.afya.payload.response.CreateReminderBulkResponse.class))),
    ...
    @Operation(summary = "Get reminders by status",
            description = "Retrieves user's reminders filtered by status with full date-time information. " +
                    "**Use Cases:** " +
                    "• TAKEN reminders for adherence tracking " +
                    "• MISSED reminders for follow-up " +
                    "• SCHEDULED reminders for upcoming notifications " +
