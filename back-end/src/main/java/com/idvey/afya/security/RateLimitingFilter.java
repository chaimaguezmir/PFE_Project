...
public class RateLimitingFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
		...
		if (bucket.tryConsume(1)) {
			...
			httpResponse.getWriter()
				.write("{\"error\":\"Too Many Requests\",\"message\":\"Rate limit exceeded\",\"status\":429}");
		}
	}
	...
	// Clean up old entries periodically (you might want to implement this with
	// @Scheduled)
	public void cleanupOldEntries() {
		// Remove entries older than 1 hour to prevent memory leaks
		// This is a simple implementation - in production, consider using Redis or
		// similar
		if (cache.size() > 10000) {
			cache.clear();
