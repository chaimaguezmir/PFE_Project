...
public class TreatmentService {
	...
	public List<TreatmentResponse> getAll(UUID userId) {
		return treatmentRepository.findByUser_Id(userId).stream().map(this::toResponse).toList();
	}
	...
	private TreatmentResponse toResponse(Treatment treatment) {
		return new TreatmentResponse(treatment.getId(), treatment.getName(), treatment.getStartDate(),
				treatment.getEndDate(), treatment.getDiseases().stream().map(Disease::getName).toList());
	}
