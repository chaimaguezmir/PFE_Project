...
public class GroupService {
    ...
    public UUID createGroup(UUID creatorId, String name) {
        ...
        if (memberRepo.existsByUser_IdAndGroup_Name(creatorId, name)) {
            ...
            throw new IllegalStateException(
                    "You already have a group named '" + name + "'");
        }
    ...
    public void addUserToGroup(UUID groupId, UUID actorId, String usernameToAdd) {
        System.out.println("[Service] addUserToGroup called by user " + actorId + " to add '" + usernameToAdd + "' to group " + groupId);
        GroupMember actor = memberRepo
                .findByGroup_IdAndUser_Id(groupId, actorId)
    ...
    public void removeUserFromGroup(UUID groupId, UUID actorId, UUID targetId) {
        System.out.println("[Service] removeUserFromGroup called by user " + actorId + " to remove user " + targetId + " from group " + groupId);
        GroupMember actor = memberRepo
                .findByGroup_IdAndUser_Id(groupId, actorId)
                .orElseThrow(() -> new NoSuchElementException("You are not a member of this group"));
        if (actor.getRole() != GroupRole.MANAGER) {
        ...
        GroupMember target = memberRepo
                .findByGroup_IdAndUser_Id(groupId, targetId)
    ...
    public void renameGroup(UUID groupId, UUID actorId, String newName) {
        System.out.println("[Service] renameGroup called by user " + actorId + " for group " + groupId + " to name '" + newName + "'");
        GroupMember actor = memberRepo
                .findByGroup_IdAndUser_Id(groupId, actorId)
                .orElseThrow(() -> new NoSuchElementException("You are not a member of this group"));
        if (actor.getRole() != GroupRole.MANAGER) {
        ...
        if (memberRepo.existsByUser_IdAndGroup_Name(actorId, newName)
                && !actor.getGroup().getName().equals(newName)) {
            System.out.println("[Service] Duplicate group name on rename for user " + actorId + ": '" + newName + "'");
            throw new IllegalStateException(
                    "You already have a group named '" + newName + "'");
        }
    ...
    public void leaveGroup(UUID groupId, UUID userId) {
        ...
        var firstJoined = remaining.stream()
                .min(Comparator.comparing(GroupMember::getJoinedAt))
                .orElseThrow();
        firstJoined.setRole(GroupRole.MANAGER);
        memberRepo.save(firstJoined);
        System.out.println("[Service] User " + firstJoined.getUser().getId() + " promoted to MANAGER for group " + groupId);
    }
