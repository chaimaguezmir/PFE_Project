...
class WelcomeScreenCubit extends Cubit<WelcomeScreenState> {
  WelcomeScreenCubit({required this.reminderRepository})
      : super(const WelcomeScreenState());

  final ReminderRepository reminderRepository;

  // Helper: only emit if cubit is still alive
  void safeEmit(WelcomeScreenState newState) {
    if (!isClosed) {
      emit(newState);
    }
  }

  /// Load initial welcome data
  Future<void> loadWelcomeData() async {
    if (isClosed) return;
    safeEmit(state.copyWith(status: FormzSubmissionStatus.inProgress));
    try {
  ...
  Future<void> fetchReminders() async {
    ...
    try {
      final result = await reminderRepository.getRemindersWithMedications();
      if (isClosed) return;
  ...
  Map<String, int> getDayStatistics() {
    ...
    int missed = dayReminders.where((r) => r.status.toUpperCase() == 'MISSED').length;
  ...
  void reset() {
    if (isClosed) return;
    safeEmit(const WelcomeScreenState());
  }
}
