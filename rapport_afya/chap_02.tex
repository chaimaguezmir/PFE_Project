...
\begin{itemize}
    ...
    \item \textbf{Spring Boot 3.4 :} Framework Java pour le développement d'applications backend robustes et scalables. Il simplifie la configuration et le déploiement des applications Spring, et intègre des modules tels que Spring Security pour l'authentification JWT, Spring Data JPA pour la persistance des données, et Hibernate comme ORM.
    ...
    \item \textbf{MariaDB :} Système de gestion de base de données relationnelle pour stocker toutes les données de l'application.
...
\begin{itemize}
    \item \textbf{Flutter 3.8+ :} Framework Google pour le développement d'applications mobiles cross-platform (Android/iOS) avec une seule base de code. L'application utilise des packages Flutter tels que flutter\_bloc pour la gestion d'état, GetIt pour l'injection de dépendances, Dio pour les appels HTTP, et go\_router pour la navigation.
